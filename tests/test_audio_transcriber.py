...
def test_stream_url_thread_basic_operation(monkeypatch):
    ...
    # Create fake PCM data (1 second worth of audio)
    ...
    assert isinstance(segment2, AudioSegment)
...
def test_stream_url_thread_stop_event(monkeypatch):
    ...
    def read_with_stop_check(size):
        # Check stop event and return data if not stopped
        if stop_event.is_set():
            return b''
        time.sleep(0.01)  # Small delay to allow stop event to be processed
        return fake_pcm

    mock_stdout.read.side_effect = read_with_stop_check
...
def test_stream_url_thread_queue_limiter_drops(monkeypatch):
    ...
    audio_queue = queue.Queue()
    stop_event = threading.Event()
    limiter = audio_transcriber.QueueBacklogLimiter(max_seconds=2.0, source_label="test")

    # Fill limiter to trigger drops
    limiter.try_add(2.5)  # Over limit
    ...
    def mock_read(size):
        ...
        if call_count[0] <= 2:
            ...
            # Stop the thread after reading chunks
            stop_event.set()
            return b''
    ...
    # Count only AudioSegments (not TranscriptionNotice)
    audio_segments = []
    ...
    # Should have dropped at least one chunk due to backlog
    assert len(audio_segments) < 2
    # Limiter should have tracked dropped seconds
...
def test_stream_url_thread_retry_on_error(monkeypatch):
    ...
    def mock_stream_url_failing(url):
        ...
        if call_count[0] == 1:
            ...
            mock_stream.__enter__ = Mock(return_value=mock_stdout)
            mock_stream.__exit__ = Mock(return_value=False)
            stop_event.set()  # Stop after retry
            return mock_stream
...
def test_stream_url_thread_timestamp_continuity(monkeypatch):
    ...
    # Create 3 chunks of 1 second each
    fake_pcm = np.zeros(audio_transcriber.TARGET_SAMPLE_RATE, dtype=np.int16).tobytes()
...
def test_stream_url_thread_empty_chunks_handled(monkeypatch):
    """Test that empty chunks (EOF) terminate stream gracefully."""
    audio_queue = queue.Queue()
    stop_event = threading.Event()

    mock_stdout = MagicMock()
    # Return empty bytes immediately (EOF)
    mock_stdout.read.return_value = b''
    ...
    # Should eventually stop and set stop_event
    time.sleep(0.1)
    stop_event.set()
    thread.join(timeout=2.0)
